...
**Сейчас:** 21 видеокарта, для каждой свой профиль CPU/RAM/Screen чтобы не было абсурдных сочетаний (например RTX 4070 + 4 потока). Пул смещён в сторону бюджетных ноутбуков 2018-2024 — такие машины преобладают у реальных людей.

Фактическое распределение (замер на 3000 сгенерированных профилей):

| Параметр | Распределение |
|---|---|
| Класс | ноутбуки 63% / десктопы 37% |
| Экран | **1920x1080 — 88%**, 2560x1440 — 12% (как в реальной статистике) |
| CPU (потоки) | 8 — 44%, 4 — 20%, 12 — 16%, 6 — 14%, 2 — 6% |
| RAM | 16 GB — 56%, 8 GB — 38%, 4 GB — 7% |
| Windows | 10 — 51%, 11 — 49% |

**Файл:** `session_template.py`

### 3.0. Два правила, без которых fingerprint выглядит подделкой

**Поле CPU — это потоки, а не ядра.** Браузер отдаёт `navigator.hardwareConcurrency` = логические процессоры. Поэтому:
- `8` = 4 ядра / 8 потоков (i5-8250U, Ryzen 5 3500U) — самый частый ноутбук
- `12` = 6c/12t (i5-11400H) — игровые ноуты
- `4` = i3 / Pentium, `2` = Celeron
- `6` = 6c/6t — типично для **десктопов** (i5-9400F), в ноутбуках почти не встречается

**1440p только там, где оправдан.** Офисные машины и массовые ноутбуки в жизни почти всегда FHD. 2560x1440 оставлен для игровых десктопных карт (GTX 1660, RTX 3060, RX 6600 — по 50%; RTX 4070 — всегда). Если добавляешь ноутбучную карту, ставь только `1920x1080`, иначе доля 1440p поползёт вверх и станет неправдоподобной.
...
⚠️ **Имя должно строго совпадать с каталогом LS.** Каталог лежит в `session_imports/_template.xlsx`, лист **«Инструкция RU»**: ячейка `Q7` — видеокарты (231 позиция для Windows), `O7` — разрешения, `M7` — CPU, `N7` — RAM. Опечатка → LS Mass Import выкинет fingerprint в "error" rows.

**Как посмотреть каталог не открывая Excel:**
```powershell
cd C:\warmup
python -c "import openpyxl,re; w=openpyxl.load_workbook('session_imports/_template.xlsx'); s=str(w['Инструкция RU']['Q7'].value).split('\"windows\"')[1]; print('\n'.join(re.findall(r'\"((?:Nvidia|Intel|AMD),[^\"]+)\"', s)))"
```
