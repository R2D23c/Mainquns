...
**Связанные настройки:** `[api] urls_total_target_min/max` в этой ветке трактуются **как per-profile** (по умолчанию 150-250 → ~2.5-3.5k cookies на профиль). При 3 профилях суммарно 450-750 URL ≈ 5-6 часов wall-clock. При 4-5 профилях подними бюджет времени или опусти target до ~100-150.
