...
7. **Мёртвый профиль НЕ блокирует машину.** Активный выбирается из
   упорядоченной очереди `candidates` (наименее прогретый первым), и
   если сессия не резолвится — берём следующего. Без этого одна
   удалённая сессия вешала бы машину навсегда: её счётчик 0 → ratio 0
   → её выбирало бы каждый цикл, остальные профили не прогрелись бы
   никогда. Ретрай (220с) только для ПЕРВОГО кандидата, остальные
   fail-fast — иначе N мёртвых профилей = N×220с впустую. Пропущенные
   попадают в ⚙️ push (low), не молча. Если не резолвится НИ ОДИН —
   ApiError с дословным текстом последней ошибки: по её подстрокам
   ("GET /sessions" + 10061/refused) error-handler отличает «LS API
   умерла» от «сессий правда нет».

8. **`import_button` timeout масштабируется от числа профилей:**
   `600 + 240×(N-1)`, потолок 2400с. Pre-import cloud sync валидирует
   fingerprint КАЖДОЙ строки xlsx — на 2 профилях наблюдалось 6 мин
   (25.07.2026), т.е. фикс. 600с был на грани и на 5-10 профилях
   гарантированно ловил бы timeout. Одиночный профиль = ровно 600с,
   как в main. UI install при этом может идти >45 мин — конфликта нет:
   IgnoreNew пропустит тик, watchdog при Running не вмешивается.

6. **Ретрай lookup'а сессии** (`_SESSION_LOOKUP_BACKOFFS = [10,30,60,120]`).
   25.07.2026 на 5-профильной машине: сессия видна в GUI со статусом
   warmup, но `GET /sessions` её не возвращает → цикл падал ⚠️ high,
   хотя через час всё шло само. Причина: LS-cloud отдаёт каталог не
   атомарно, а lookup был ЕДИНСТВЕННЫМ шагом цикла без ретрая (ping,
   409-pyramid, connection-refused обвешаны). Цепочка усугубила:
   пауза между циклами упала с ~8 мин до 90с, слака на «LS дописывает
   состояние» не осталось. Коллизия имён ретраем НЕ лечится — падает
   сразу (нужен оператор). Догон export'ов зовёт с `retry=False`.
